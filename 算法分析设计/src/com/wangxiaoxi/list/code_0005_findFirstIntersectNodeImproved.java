...
public class code_0005_findFirstIntersectNodeImproved {

    /**
     * 寻找单向链表的第一个相交点，思路：
     * 需要考虑单向链表是无环还是有环，相交的情况一共有几种（注意是单向链表一个next指针只能指向一个节点）：
     * 1,无环和无环，如果相交则终点end一定相同
     * 2,有环和无环，一定不存在相交，如果相交，则无环的会变成有环，自相矛盾
     * 3,有环和有环，一共有三种情况，入环节点相同，入环节点不同，没有相同的入环节点
     *
     * 4,方法集合改进版--getIntersectNode
     */
    ...
    //得到两个链表的相交点（将链表有环(A)，无环(B)，以及A,B的各种情况组合，集合成一个方法，并最终返回第一个相交节点）
    public static Node getIntersectNode(Node head1,Node head2){
        if(head1 == null || head2 == null){
            return null;
        }
        //判断这两个链表的结构
        Node c1 = getEnterLoopNode(head1);
        Node c2 = getEnterLoopNode(head2);

        //如果两个链表都是无环的
        if(c1 == null && c2 == null){
            return bothIsNotLoop(head1,head2);
        }
        //如果两个链表都是有环的
        else if(c1 != null && c2 != null){
            return bothAreLoop(head1,head2);
        }else{
            return null;
        }
    }
    ...
    public static boolean isloop(Node head){
        ...
        Node slowP = head;
        Node fastP = head.next;   //一开始fastP需要在slowP前面，因为下面需要判断两节点重合，所以起点不同
        ...
        while(slowP != null && fastP != null){
            slowP = slowP.next;
            if(fastP.next != null){
                fastP = fastP.next.next; // A.next.next在无环单链表中很容易导致空指针异常！！，有环可以不加判断条件
            }else{
                break;
            }
    ...
    public static Node bothAreLoop(Node head1,Node head2){
    ...
    //得到入环节点,实现思路:
    //慢指针走1步，快指针走2步
    //当快指针和慢指针重合时，快指针回到head位置，慢指针向前走1步，快指针和慢指针的步数都为1
    //如果快指针和满指针再次相遇，则给节点为入环节点
    public static Node getEnterLoopNode(Node head){
        ...
        while(slowP != fastP){
            slowP = slowP.next;
            fastP = fastP.next.next; //注意 A.next.next在无环单链表中很容易导致空指针异常！！
...
//        int temp = size + 1; //便于产生随机的[0,size]之间的随机大小的数组
    ...
    //以数组的形式返回两个无环链表的头指针
    public static Node[] getTwoListWithoutLoop(){
...
//        list2_head = createLoop(list2_head);
        ...
        return new Node[] {list1_head,list2_head};
    }


    //以数组的形式返回2个有环链表:两个环，相交点相同
    public static Node[] getTwoLoop1(){
    ...
    //以数组的形式返回2个有环链表:两个环，相交点不同
    public static Node[] getTwoLoop2(){
...
//        Node[] nodes = getTwoListWithoutLoop();   //得到两条无环链表，并且可以得到第一个相交节点
//        Node[] nodes = getTwoLoop1();   //得到两条有环链表(1)，并且可以得到第一个相交节点
        Node[] nodes = getTwoLoop2();   //得到两条有环链表(2)，并且可以得到第一个相交节点

        Node list1_head = nodes[0];
        Node list2_head = nodes[1];

        /**判断两条链表的相交点**/
        Node intersectNode = getIntersectNode(list1_head,list2_head);
